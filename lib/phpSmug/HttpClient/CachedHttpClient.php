...
namespace phpSmug\HttpClient;

use phpSmug\HttpClient\Cache\CacheInterface;
use phpSmug\HttpClient\Cache\FilesystemCache;

/**
 * Performs requests on phpSmug API using If-Modified-Since headers.
...
class CachedHttpClient extends HttpClient
{
    ...
    {
        if (null === $this->cache) {
            $this->cache = new FilesystemCache($this->options['cache_dir'] ?: sys_get_temp_dir().DIRECTORY_SEPARATOR.'phpSmug-cache');
        }
